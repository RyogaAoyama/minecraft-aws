...
fabric-api          # 版指定なし → 1.21.10/fabric 向けの最新 release を自動選択
...
- 対象は **MC 1.21.10 / Fabric 固定**（サブバージョン混在不可）。`fetch-mods.sh` のデフォルトも同じです。
...
2. `slug==version_id` ならその version、`slug` のみなら Modrinth API で
   `1.21.10 / fabric` 向け候補を取得し、`release` 優先・公開日降順で最新を選ぶ。
