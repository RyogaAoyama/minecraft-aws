...
スポット EC2 + S3 母艦運用の Minecraft(Fabric 1.21.10) サーバーの構築・運用手順です。
...
> 1.21.10 に更新**しないと接続できません。MOD 更新時はプレイヤー全員のクライアント側更新も
> 合わせて行ってください。
...
- 課金されるのは「起動中のみ」。`c8g.xlarge` スポット相当の従量 + EBS(gp3 30GB) + EIP(未使用時のみ少額)。
