...
- **0人が10分連続**（毎分×10回）で、ワールドを保存→S3同期→自インスタンスを ASG から終了し
  desired を 0 に戻します（課金停止）。
- 1人でもオンラインならカウンタはリセットされます。

### 定期保存

- `world-sync.timer` が5分毎に `save-all` → S3 差分同期を行います。
- スポット中断時は `spot-watch.service`（3秒間隔ポーリング）と
  `minecraft.service` の `ExecStop` の二段でワールド保存を担保します。

### コスト感

- 課金されるのは「起動中のみ」。`c6gd.xlarge` スポット相当の従量 + EBS(gp3 10GB, root) + EIP(未使用時のみ少額)。
  ワールドデータは NVMe instance store(237GB, 無料) に配置し、停止時に S3 へ退避する。
...
- **world 肥大時の同期遅延**: ワールドが極端に大きくなると `s3 sync` が
  ...
  差分 sync + 5分毎の定期 sync で通常の差分を小さく保つ前提です。
...
- **NVMe ephemeral 消失**: ワールド/サーバー資産は NVMe instance store に配置されており、
  インスタンス終了で完全消失する。停止/中断時に save-and-sync が S3 へ退避することで保全する設計
  (TimeoutStopSec=110 + spot-watch 3秒間隔 + standby promotion 機構)。
  ワールドサイズが 10GB を超えると Spot 中断猶予 2 分内に sync が間に合わない可能性があるため、
  CloudWatch で diskio (nvme1n1) のサイズ増加を観測しておくこと。
